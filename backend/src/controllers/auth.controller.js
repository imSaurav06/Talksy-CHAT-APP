...
export const signup = async (req, res) => {
  ...
  try {
    ...
    if (newUser) {
      //generate jwt token and iska code util.js me likha hai
    ...
    console.log("Error in signup backend controller", error.message);
...
export const login = async (req, res) => {
  ...
  try {
   ...
   // check if user exists or not
    ...
    // token generation using util.js file
...
// for update profile we need to use using service call {cloudinary}
export const updateProfile = async (req, res) => {
  try {
    //fetch profilePic from ../models/user.model.js"file
    const { profilePic } = req.body;
    const userId = req.user._id;

    // if not available then trowe the message
    if (!profilePic) {
